...
.단계
. Connector를 배포하는 Google Cloud 사용자에게 에 대한 권한이 있는지 확인합니다 https://occm-sample-policies.s3.amazonaws.com/Setup_As_Service_3.7.3_GCP.yaml["Google Cloud용 커넥터 배포 정책"^].
+
...
+
.. https://cloud.google.com/iam/docs/creating-custom-roles#iam-custom-roles-create-gcloud["GCP에서 역할을 생성합니다"^] 여기에는 에 정의된 권한이 포함됩니다 https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.10_GCP.yaml["Google Cloud에 대한 Cloud Manager 정책"^]. gcloud 명령줄을 사용해야 합니다.
+
...
* https://occm-sample-policies.s3.amazonaws.com/Setup_As_Service_3.7.3_GCP.yaml["YAML 파일에 있는 권한"^]
