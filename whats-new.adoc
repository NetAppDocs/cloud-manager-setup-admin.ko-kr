...
link:reference-networking-cloud-manager.html#outbound-internet-access["필요한 끝점 목록을 봅니다"].
...
image:screenshot-connector-disk-encryption.png["AWS에서 커넥터를 생성할 때 디스크 암호화 옵션을 보여 주는 스크린샷"]
...
link:task-managing-netapp-accounts.html["타임라인을 Tenancy 서비스로 필터링하는 방법에 대해 알아보십시오"].
